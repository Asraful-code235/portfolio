...
(function ($) {
  $.fn.nevoslider = function (options) {
    ...
    function setCursor(e) {
      ...
      else slider.attr("data-direction", "right");
    }
    ...
    function slideTo(direction) {
      ...
      if (activeSlide.has("video")) activeSlide.find("video").trigger("pause"); // Pause of previous
      ...
      if (activeSlide.has("video")) activeSlide.find("video").trigger("play"); // Play on new

      if (activeSlide.hasClass("light-text") && slider.is(":first-child"))
        $body.addClass("header-inverted");
      else $body.removeClass("header-inverted");
    }

    function slideAnimation() {
      curPos = -(curSlide - 1) * sliderWidth;
      TweenMax.to(slides, 1, {
        x: curPos,
