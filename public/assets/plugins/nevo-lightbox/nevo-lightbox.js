...
(function ($) {
  nevoLightbox = function (options) {
    ...
    var $body = $("body");
    var $lightbox = $(
      '<div id="lightbox"><div class="bg"><div class="loader"></div></div><div class="close-lightbox"></div><div class="prev-lightbox-item"></div><div class="next-lightbox-item"></div></div>'
    );
    var curIndex;
    var curItem;
    var galleryTotal;
    ...
    $body.on("click", ".prev-lightbox-item", function (e) {
      ...
      e.stopImmediatePropagation();
      curIndex = curIndex - 1;
      if (curIndex < 0) curIndex = galleryTotal;
    ...
    $body.on("click", ".next-lightbox-item", function (e) {
      ...
      e.stopImmediatePropagation();
      curIndex = curIndex + 1;
      if (curIndex > galleryTotal) curIndex = 0;
    ...
    function loadFirstItem() {
      var img = new Image();
      img.src = $body
        .find('.lightbox[data-index="' + curIndex + '"]')
        .attr("href");
    ...
    function loadNewItem(newIndex, direction) {
      var img = new Image();
      img.src = $body
        .find('.lightbox[data-index="' + newIndex + '"]')
        .attr("href");

      var oldItem = curItem;
      initScale = 0.6;

      if (direction == "prev") (toPosition = "200%"), (fromPosition = "-200%");
    ...
    function initLightbox(el) {
      ...
      $lightbox.find(".bg").fadeIn(500, function () {
        (toPosition = 0), (fromPosition = 0), (initScale = 0.6);
        loadFirstItem();
      });
      ...
      $lightbox.find(".bg, .close-lightbox").on("click", function (e) {
        ...
        if ($body.on("mousemove")) $body.off("mousemove");
      });
    }
    ...
    function setDimensions(el) {
      ...
      if (yElFactor > 0) yPos = 0;
      if (xElFactor > 0) xPos = 0;
    ...
    function thumbHandler(thumbClone, title, subtitle) {
      var titleHTML = '<div class="title">' + title + "</div>";
      var subtitleHTML = '<div class="sub">' + subtitle + "</div>";
      var titleElement = $(titleHTML);
      var subtitleElement = $(subtitleHTML);
      thumbClone.find(".caption").append(titleElement);
      thumbClone.find(".caption").append(subtitleElement);
    ...
    function parallaxItem() {
      $body.on("mousemove", function (e) {
        ...
        TweenMax.to("#lightbox .zoomed", 0.3, {
          transform: "translate3d(" + xPos + "px, " + yPos + "px,0)",
        });
      });
    }
